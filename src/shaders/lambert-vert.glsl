...
float fbm( vec3 p ) { // in [0,1]
    ...
    f  = 0.4980*noise( p ); p = m*p*1.0 + d_time;
    f  = 0.0020*noise( p ); p = m*p*5.0 + d_time;
    f += 0.2500*noise( p ); p = m*p*0.3 + d_time;
    f += 0.1250*noise( p ); p = m*p*5.01;
    f += 0.0625*noise( p );
    return f;
}
// END
...
void main()
{
    ...
    // flametails = flametails + lvl_noise * 3.0 * (clamp(lvl_2, 0.4, 2.0) - 0.5);
    ...
    vec4 disp_4d = vec4(modelposition3 * (1.0 + disp * 2.0), 1.0);
